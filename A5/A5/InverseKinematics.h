...
bool CCDIK(vec3 position, int endEffectorIndex, int chainRootIndex, Bone** bones)
{
	...
	float distance = sqrt(get_squared_dist(position, bones[chainRootIndex]->getGlobalPosition()));
	float maxReach = 0;

	for (int i = bone_index; i >= chainRootIndex; i--)
	{
		maxReach += sqrt(get_squared_dist(bones[i + 1]->getGlobalPosition(), bones[i]->getGlobalPosition()));
	}

	if (distance > maxReach)
	{
		cout << "Distance > MaxReach" << endl;
		vec3 endDirection = position - bones[chainRootIndex]->getGlobalPosition();
		endDirection = endDirection / distance;
		endDirection = endDirection * maxReach;
		position = endDirection + bones[chainRootIndex]->getGlobalPosition();
	}

	do
	{
		rootPos = bones[bone_index]->getGlobalPosition();
		curEnd = bones[endEffectorIndex]->getGlobalPosition(); // Position of the end effector
